      ...
      body.innerHTML =
        ...
        (isFailed
          ? '<div class="r-alert-item err" style="margin-bottom:12px"><i class="fa-solid fa-circle-xmark"></i> ' +
            esc(e.error_msg || e.error || e.message || e.details || e.reason || 'Tiada error message dalam log ini — check subcollection vibes_errors') +
            '</div>'
          : '') +
    ...
    RS.db.collection('devices/' + hostname + '/vims_results')
      ...
      .get().then(function(snap) {
        ...
        box.innerHTML =
          ...
          '<span class="r-badge r-badge-info">not_found</span> DO tiada dalam portal' +
          '</div>' +
    ...
    view.innerHTML =
      '<div class="r-panel">' +
        ...
        '<div class="r-legend">' +
          ...
          '<span class="r-badge r-badge-warn">skipped</span> Bot skip — masalah teknikal apps &nbsp;' +
          '<span class="r-badge r-badge-info">not_found</span> DO tiada dalam portal VIMS' +
        '</div>' +
        '<div class="r-info-box warn">' +
          '<i class="fa-solid fa-triangle-exclamation"></i>' +
          '<div><strong>Proof:</strong> <code>skipped</code> = bot issue (apps problem). ' +
          '<code>not_found</code> = DO tiada dalam portal (bukan salah bot). ' +
          'Gunakan bukti ni untuk verify laporan user.</div>' +
        '</div>' +
