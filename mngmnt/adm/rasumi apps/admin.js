...
function categoryBadge(cat) {
  const map = {
    guard_exhausted: 'danger', batch_skip: 'warn', upload_retry_exhausted: 'danger',
  ...
  db.collectionGroup('vibes_errors')
    ...
    .get().then(snap => {
      ...
      new Chart(ctx, {
        ...
        options: {
          responsive: true, plugins: { legend: { labels: { color: '#c0c5d2', font: { size: 10 } } } },
          scales: {
            x: { ticks: { color: '#8b91a2', font: { size: 10 } }, grid: { color: 'rgba(192,132,252,0.07)' } },
  ...
  vc.innerHTML = `
    ...
    <div class="panel" style="padding:12px 14px;margin-bottom:14px;border-left:3px solid var(--warn)">
      ...
      <span style="color:var(--text-1);font-size:12px;margin-left:6px">
        <b class="text-warn">Bot Skipped</b> = Host daemon execution failure — ingestion sequence aborted by client application.
        <b class="text-info" style="margin-left:12px">Not in Portal</b> = DO/INV record unavailable in VIMS Portal.
      </span>
    </div>
